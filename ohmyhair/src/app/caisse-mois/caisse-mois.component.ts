...
@Component({
  selector: 'app-caisse-mois',
  templateUrl: './caisse-mois.component.html',
  styleUrls: ['./caisse-mois.component.css']
})
export class CaisseMoisComponent implements OnInit {
  load: boolean;
  date: string;
    private sub: any;
    ClientNames: string[];
  ...
  constructor(private route: ActivatedRoute,private http: Http, private _location: Location,private ds:DataServiceService) { 
  ...
  async generatePDF()
  {
      ...
      var columns=["Date","Client","Total","Détails"];
      var rows: [
      ...
      for( let element of this.ClientServices)
      {
        ...
        const clientName= await this.getClientName(element.id_client);
        rows[index]=[this.ds.prettyDate(element.timestamp.toString()), clientName,this.getTotal(index).toFixed(2)+" CHF",details];
        index++;
      }
      ...
      doc.text(100, 30, "Rapport de caisse pour le mois de "+this.getMonthName(this.date));
      doc.setFontSize(10);
      doc.text(100,50, "Total du mois: "+this.getRevenue().toFixed(2)+" CHF");
      ...
      doc.save("Rapport_caisse_"+this.getMonthName(this.date)+"_"+this.getYear(this.date)+".pdf");
  }
  backClicked() {
        this._location.back();
  }
 ...
 async loadClients()
  ...
  {
      const res = await this.http.get(this.ds.month_services_URL+"/"+this.date).first().map((res)=>res.json()).toPromise();
  ...
  getYear(date):string{
    var dateD:Date= new Date(date);
    return dateD.getFullYear().toString();
  }
  getMonthName(date):string
  {
    var monthNames=['janvier','février','mars','avril','mai','juin','juillet','août','septembre','octobre','novembre','décembre'];
    var dateD:Date=new Date(date);
    return monthNames[dateD.getMonth()];
  }
